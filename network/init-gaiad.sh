...
if [ "$NODE" == "node2" ]; then
    $BINARY gentx val2 "2650000000$STAKEDENOM" --home "$CHAIN_DIR" --chain-id "$CHAINID" --moniker="val2" --ip="0.0.0.0" --node="tcp://localhost:36656" --keyring-backend test
elif [ "$NODE" == "node3" ]; then
    $BINARY gentx val3 "2350000000$STAKEDENOM" --home "$CHAIN_DIR" --chain-id "$CHAINID" --moniker="val3" --ip="0.0.0.0" --node="tcp://localhost:46656" --keyring-backend test
elif [ "$NODE" == "node4" ]; then
    $BINARY gentx val4 "2000000000$STAKEDENOM" --home "$CHAIN_DIR" --chain-id "$CHAINID" --moniker="val4" --ip="0.0.0.0" --node="tcp://localhost:56656" --keyring-backend test
elif [ "$NODE" == "node1" ]; then
    $BINARY gentx val1 "3000000000$STAKEDENOM" --home "$CHAIN_DIR" --chain-id "$CHAINID" --moniker="val1" --ip="0.0.0.0" --node="tcp://localhost:16656" --keyring-backend test
